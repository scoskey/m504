...
def powerTable(n):
  ...
  \begin{parts}
    \part Explain why the numbers $1,\ldots,p-1$ with the $\times$ operation form a \emph{group}, that is, the set is closed under the operations multiplication-modulo-$p$ and inverse-modulo-$p$.
    \vspace\fill
    \part Explain why if $a\in1,\ldots,p-1$ then the powers $a,a^2,a^3,\ldots\pmod{p}$ (the list will eventually repeat) form a \emph{subgroup}, that is, they are also closed under the operations multiplication-modulo-$p$ and inverse-modulo-$p$.
    \vspace\fill
    \part Explain why if $G$ is a group and $H$ is a subgroup, then the size of $G$ is divisible by the size of $H$. (Or look it up---it's called Lagrange's theorem.)
    \vspace\fill
